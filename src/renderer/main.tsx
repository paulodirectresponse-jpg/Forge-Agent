...
function App() {
 ...
 async function syncRemote() {
  if (!project || project.source !== 'github' || !window.forge) return;
  setBusy(true);
  try { const updated = await window.forge.projects.syncRemote(project.id); if (updated) { await refreshProjects(); setActive(updated.id); setNotice('Repositório sincronizado. Preview, testes e execução estão disponíveis.'); } }
  catch (error) { setNotice(String(error)); }
